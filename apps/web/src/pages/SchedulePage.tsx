...
export function SchedulePage({ sessions, setSessions, clients }: SchedulePageProps) {
  ...
  const handleCopyReviewLink = async () => {
    const reviewUrl = `${window.location.origin}/booking/${TENANT_SLUG}/review`;
  ...
  return (
    <div className="flex-1 min-w-[1192px] flex flex-col bg-[#F8FAFC]">
      ...
      <header className="h-[80px] bg-white border-b border-[#E2E8F0] px-[26px] flex items-center justify-between gap-5 shrink-0">
        <div>
          <span className="os-eyebrow block">PRACTICE CALENDAR</span>
          <h1 className="text-[20px] font-bold tracking-[-0.02em] text-[#0F172A]">Schedule</h1>
        </div>
      ...
      {/* Main Workspace */}
      <main className="p-[24px_26px_30px] space-y-4 flex-1 flex flex-col">
        {/* Title Bar & Legend */}
        ...
        {viewMode !== 'month' ? (
          /* Week & Day Grid View */
          <div className="os-card overflow-hidden flex-1 border border-[#E2E8F0] grid" style={{ gridTemplateColumns: `64px repeat(${activeDays.length}, 1fr)` }}>
            {/* Hour Gutter */}
